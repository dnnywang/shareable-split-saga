...
const PurchaseForm = ({ tripId, onClose }: PurchaseFormProps) => {
  ...
  useEffect(() => {
    if (currentTrip) {
      ...
      setPayerSelections([{ userId: currentTrip.participants[0].id, amount: 100 }]);
  ...
  const handlePayerAmountChange = (userId: string, amount: number) => {
  ...
  const createPurchase = async () => {
    ...
    const paidBy = payerSelections.map(payer => ({
      userId: payer.userId, // Change participantId to userId
    ...
    const splitBetween = splitSelections.map(split => ({
      userId: split.userId, // Change participantId to userId
        ...
        <div className="space-y-2">
          ...
          {currentTrip.participants.map(participant => (
            <div key={participant.id} className="flex items-center justify-between">
              ...
              <div className="w-24">
                <Slider
                  defaultValue={payerSelections.find(p => p.userId === participant.id)?.amount || 0}
                  max={100}
                  step={1}
                  onValueChange={(value) => handlePayerAmountChange(participant.id, value[0])}
                />
              </div>
              <span>{payerSelections.find(p => p.userId === participant.id)?.amount || 0}%</span>
            </div>
          ))}
          <p className="text-sm text-muted-foreground">Total Paid: {totalPaid}%</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          {currentTrip.participants.map(participant => (
            <div key={participant.id} className="flex items-center justify-between">
              ...
              <div className="w-24">
                <Slider
                  defaultValue={splitSelections.find(s => s.userId === participant.id)?.amount || 0}
                  max={100}
                  step={1}
                  onValueChange={(value) => handleSplitAmountChange(participant.id, value[0])}
                />
              </div>
              <span>{splitSelections.find(s => s.userId === participant.id)?.amount || 0}%</span>
            </div>
          ))}
          <p className="text-sm text-muted-foreground">Total Split: {totalSplit}%</p>
        </div>
      </CardContent>
